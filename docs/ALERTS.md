...
- `reboot_cause` carries the categorical `cause` and the agent's `sources`,
  followed by the correlation evidence `reboot_flagged_updates`,
  `recent_installs`, `system_reboot_required`, `scanned_at`, and
  `snapshot_received_at`. It is `null` for a non-reboot alert, a missing policy
  revision, or when the agent reports no sources and no update inventory
  exists. `snapshot_received_at` is `null` when the verdict rests on source
  flags alone.

`cause` is one of `update_caused`, `not_update_caused`, or `unknown`. It is
`update_caused` exactly when the agent reports
`WindowsUpdate\Auto Update\RebootRequired` set, because that is the only one
of the three sources that means an installed update is waiting on a restart.
`sources` mirrors the result detail described in
[`MONITORING.md`](MONITORING.md) and is `null` whenever the agent did not
report usable source flags.
...
Agents released before v0.1.8 do not report a `sources` key in result detail.
Its absence means `cause` is `unknown` and the dashboard says "Cause
unavailable" while showing any correlated inventory beneath it. It must never
be interpreted as "not update-related." Source attribution does not change the
evidence contract above: the correlated lists stay correlational and stay
labeled as such next to the categorical verdict.
