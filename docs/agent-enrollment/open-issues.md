...
| ENR-013 | Multi-tenant isolation ([#66](https://github.com/lcolon231/rmm-agent/issues/66)) | Clients/sites are now authorization tenants via operator-client memberships + platform admin; server-side default-deny scoping in `app/core/tenant_scope.py`. | Cross-tenant access fails closed as 404; covered by `tests/test_tenant_authorization.py`. | Server side implemented; dashboard membership UI delivered API-first (follow-up). See `docs/TENANT-AUTHORIZATION.md`. | Architecture | In-Progress | M3 |
