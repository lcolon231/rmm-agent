...
| ENR-013 | Multi-tenant isolation ([#66](https://github.com/lcolon231/rmm-agent/issues/66)) | Clients/sites are not authorization tenants; roles are global. | A non-admin operator can access all customers. | Add operator-client memberships and mandatory server-side scoping before calling the product multi-tenant. Memberships and the platform-admin flag exist as of revision `0037`; the server-side scoping that enforces them does not, so the product is still not multi-tenant. | Architecture | In-Progress | M3 |
