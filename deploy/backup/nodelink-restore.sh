...
WANT_ENC_SHA="$(python3 -c "import json,sys; print(json.load(open(sys.argv[1]))['encrypted_sha256'])" "$MANIFEST")"
...
WANT_PLAIN_SHA="$(python3 -c "import json,sys; print(json.load(open(sys.argv[1]))['plaintext_sha256'])" "$MANIFEST")"
...
WANT_REV="$(python3 -c "import json,sys; print(json.load(open(sys.argv[1]))['schema_revision'])" "$MANIFEST")"
