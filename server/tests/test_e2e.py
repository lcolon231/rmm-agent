...
async def _enroll(c) -> tuple[str, str, str]:
    ...
    enr = (
        await c.post(
            "/enroll",
            json={"enrollment_token": et["token"], "hostname": "PC1", "os": "windows"},
        )
    ).json()
...
async def test_enroll_and_heartbeat(client):
    ...
    r = await client.post(
        "/heartbeat",
        json={"cpu_percent": 10.0, "mem_percent": 20.0, "disk_percent": 30.0},
        headers=auth,
    )
...
async def test_command_dispatch_pickup_and_signature(client):
    ...
    ack = (await client.post("/heartbeat", json={"cpu_percent": 5.0}, headers=auth)).json()
    ...
    msg = canonical_command_bytes(cmd["id"], agent_id, cmd["kind"], cmd["payload"])
...
async def test_tampered_signature_rejected_by_agent(client):
    ...
    ack = (await client.post("/heartbeat", json={"cpu_percent": 1.0}, headers=auth)).json()
    cmd = ack["pending_commands"][0]

    pub = load_pem_public_key(pub_pem.encode())
    # Tamper with the payload the agent would execute.
    forged = canonical_command_bytes(cmd["id"], agent_id, cmd["kind"], {"script": "rm -rf /"})
