...
export default async function MaintenanceWindowsLayout({
  children,
}: {
  ...
  if (session.kind === "unavailable") {
    return (
      <main className="enrollment-standalone-error">
        <h1>Maintenance windows are unavailable</h1>
        <p>Your session could not be verified. No protected schedule data was loaded.</p>
      </main>
    );
  }
  ...
  return (
    <DashboardSectionShell
      activePath="/maintenance-windows"
      navigation={navigation}
      navigationError={navigationError}
      operator={session.operator}
      sectionLabel="Operations"
      sectionTitle="Maintenance windows"
    >
      {children}
    </DashboardSectionShell>
  );
