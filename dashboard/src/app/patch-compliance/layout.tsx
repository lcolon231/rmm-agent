...
export default async function PatchComplianceLayout({ children }: { children: React.ReactNode }) {
  const session = await getDashboardSession();
  if (session.kind === "anonymous") redirect("/login");
  if (session.kind === "unavailable") {
    return (
      <main className="enrollment-standalone-error">
        <h1>Patch compliance is unavailable</h1>
        <p>Your session could not be verified. No endpoint report data was loaded.</p>
      </main>
    );
  }
  ...
  return (
    <DashboardSectionShell
      activePath="/patch-compliance"
      navigation={navigation}
      navigationError={navigationError}
      operator={session.operator}
      sectionLabel="Fleet assurance"
      sectionTitle="Patch compliance"
    >
      {children}
    </DashboardSectionShell>
  );
