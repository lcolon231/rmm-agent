...
export default async function PatchPoliciesLayout({ children }: { children: React.ReactNode }) {
  const session = await getDashboardSession();
  if (session.kind === "anonymous") redirect("/login");
  if (session.kind === "unavailable") {
    return (
      <main className="enrollment-standalone-error">
        <h1>Patch policies are unavailable</h1>
        <p>Your session could not be verified. No policy data was loaded.</p>
      </main>
    );
  }
  ...
  return (
    <DashboardSectionShell
      activePath="/patch-policies"
      navigation={navigation}
      navigationError={navigationError}
      operator={session.operator}
      sectionLabel="Fleet assurance"
      sectionTitle="Patch policies"
    >
      {children}
    </DashboardSectionShell>
  );
